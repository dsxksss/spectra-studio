...
export default function SQLiteManager({ onClose: _onClose, onDisconnect, onDragStart, connectionName }: { onClose?: () => void, onDisconnect?: () => void, onDragStart?: (e: React.PointerEvent) => void, connectionName?: string }) {
    const { t } = useTranslation();
    ...
    const [newTableCols, setNewTableCols] = useState<{ name: string; type: string; isPk: boolean; isNullable: boolean }[]>([
        { name: 'id', type: 'INTEGER', isPk: true, isNullable: false }
    ]);
    ...
    const fetchColumns = async (table: string) => {
        try {
            const cols = await invoke<string[]>('sqlite_get_columns', { tableName: table });
    ...
    const fetchCount = async (table: string) => {
        try {
            const count = await invoke<number>('sqlite_get_count', { tableName: table });
            setTotalRows(count);
        } catch (e) {
    ...
    const executeSql = async () => {
        ...
        setSqlError(null);
        try {
            const res = await invoke<string>('sqlite_execute_raw', { sql: sqlQuery });
            try {
    ...
    const handleCreateTable = async () => {
        ...
        const colsSql = newTableCols.map(c => {
            return `"${c.name}" ${c.type} ${c.isPk ? 'PRIMARY KEY AUTOINCREMENT' : ''} ${!c.isNullable ? 'NOT NULL' : ''}`;
        }).join(', ');
                ...
                <div className="flex-1 flex flex-col bg-[#121214] border border-white/5 rounded-xl overflow-hidden shadow-inner">
                    <div className="flex items-center justify-between px-4 py-2 border-b border-white/5 bg-[#18181b]/50">
                        ...
                        <button
                            ...
                            className="px-4 py-1.5 bg-cyan-500 hover:bg-cyan-600 disabled:bg-cyan-500/50 text-white rounded-md text-xs font-medium transition-all flex items-center gap-2 shadow-lg shadow-cyan-500/20"
                        >
                ...
                <div className="bg-[#121214] border border-white/5 rounded-2xl p-6 flex flex-col gap-6 shadow-xl">
                    <div className="flex flex-col gap-2">
                        ...
                        <input
                            ...
                            className="bg-[#18181b] border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-cyan-500/50 transition-all shadow-sm"
                        />
                    </div>
                    ...
                    <div className="flex flex-col gap-3">
                        <div className="flex items-center justify-between px-1">
                            ...
                            <button onClick={handleAddCol} className="flex items-center gap-2 text-xs font-semibold text-cyan-400 hover:text-cyan-300 transition-colors bg-cyan-500/10 px-3 py-1.5 rounded-lg border border-cyan-500/20">
                                ...
                                <div key={idx} className="flex items-center gap-3 bg-[#18181b] border border-white/5 p-3 rounded-xl animate-in slide-in-from-top-2 duration-200">
                                    <div className="flex-1 flex flex-col gap-1.5">
                                        ...
                                        <input
                                            ...
                                            className="bg-[#09090b] border border-white/5 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-cyan-500/30 transition-all font-mono"
                                        />
                                    </div>
                                    <div className="w-40 flex flex-col gap-1.5">
                                        ...
                                        <select
                                            ...
                                            className="bg-[#09090b] border border-white/5 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-cyan-500/30 transition-all"
                                        >
                                    ...
                                    <div className="flex items-center gap-4 px-2 pt-5">
                                        <label className="flex flex-col items-center gap-1.5 cursor-pointer group">
                                            <span className="text-[10px] text-gray-600 font-bold uppercase group-hover:text-cyan-400 transition-colors">{t('pk')}</span>
                                            <div className={`w-5 h-5 rounded border flex items-center justify-center transition-all ${col.isPk ? 'bg-cyan-500 border-cyan-500 text-white' : 'border-white/10 hover:border-white/30'}`}>
                    ...
                    <div className="flex items-center justify-end gap-3 pt-4 border-t border-white/5">
                        ...
                        <button
                            ...
                            className="px-8 py-2.5 bg-cyan-500 hover:bg-cyan-600 disabled:opacity-50 text-white rounded-xl text-sm font-bold transition-all shadow-xl shadow-cyan-500/20 flex items-center gap-3"
                        >
                        ...
                        {mode === 'edit' && (
                            <div className="flex items-center gap-2">
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-1.5 px-2 py-1 bg-cyan-500/10 text-cyan-400 hover:bg-cyan-500/20 rounded border border-cyan-500/20 transition-all font-medium text-[10px] uppercase tracking-wider"
                                >
                            ...
                            <div className="flex flex-col group">
                                ...
                                {isRenamingKey === selectedKey ? (
                                    <input
                                        ...
                                        className="bg-[#18181b] border border-cyan-500/50 rounded px-2 py-0.5 text-white font-medium text-lg outline-none focus:ring-2 focus:ring-cyan-500/20 w-64"
                                    />
                                ) : (
                                    <div className="flex items-center gap-2">
